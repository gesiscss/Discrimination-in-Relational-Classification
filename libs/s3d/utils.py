...
def visualize_cv(performance_file,
    ...
    if color_list is None:
        if cv_df.lambda_.unique().size >= 3 and cv_df.lambda_.unique().size <= 9:
            color_list = eval('palettable.colorbrewer.qualitative.Pastel1_' + str(cv_df.lambda_.unique().size) + '.mpl_colors')
        elif cv_df.lambda_.unique().size <= 20:
            nn = max(3, cv_df.lambda_.unique().size)
            color_list = eval('palettable.matplotlib.Plasma_' + str(nn) + '.mpl_colors')
        else:
...
def visualize_s3d_steps(model_folder, figsize=(8,7), color_list=None, bar_alpha=1):
    ''' visualize the increment of r-squared of s3d model
        ...
        color_list : list
            ...
            length must be the same as the number of steps (aka the number of selected features)
    '''
    df = pd.read_csv(os.path.join(model_folder,'R2improvements.csv'))
...
def visualize_s3d_model_reader(model_folder, dim, thres):
    levels = pd.read_csv(os.path.join(model_folder, 'levels.csv'))
    chosen_features = levels.loc[:,'best_feature'].values

    # `splits.csv`: bins for each feature
    splits = []
    with open(os.path.join(model_folder,'splits.csv')) as f:
        for line in f:
    ...
    ybars = []
    with open(os.path.join(model_folder,'ybar_tree.csv')) as f:
        for line in f:
            ybars.append([float(x) for x in line.split()[0].split(',')])

    #`N_tree.csv`: the number of data points in each bin
    Ns = []
    with open(os.path.join(model_folder,'N_tree.csv')) as f:
        for line in f:
...
def visualize_s3d_model(dim, splits_at_dim, cmap,
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            ax.set_xlim(min(splits_at_dim[dim-2]), max(splits_at_dim[dim-2]))
            ax.set_yscale(yscale)
            ax.set_ylim(min(splits_at_dim[dim-1]), max(splits_at_dim[dim-1]))
    ...
    if dim == 3:
        if xlab_x is None:
            xlab_x = .5 - ((len(chosen_features[dim-3]) / 2) * 0.012) * scale
        if xlab_y is None:
            xlab_y = -0.25/scale
        ...
        if xlab_x is None:
            xlab_x = .5-((len(chosen_features[0])/2)*0.008)*scale
        if xlab_y is None:
            xlab_y = -0.07/scale
        fig.text(x=xlab_x, y=xlab_y, s=chosen_features[dim-3], size=fontsize)

        if ylab_x is None:
            ylab_x = len(str(max(splits_at_dim[dim-1])))/-150.*scale  #-(.02 if (max(splits_at_dim[dim-1])) == 1.0 else .06)*scale
        if ylab_y is None:
            ylab_y = .5+((len(chosen_features[0])/2)*0.012)*scale
...
def visualize_feature_network_contruct(model_folder, node_label_mapping, color_choice, isolated_option):
    ...
    r2_improv_df = pd.read_csv(os.path.join(model_folder,'R2improvements.csv'))
    ## construct graph
    g = nx.DiGraph()
